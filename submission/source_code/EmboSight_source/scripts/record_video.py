...
def _write_video(
    ...
    try:
        ...
        for frame in frames:
            ...
            flipped = np.flipud(frame).copy()
            for _ in range(repeat):
                writer.append_data(flipped)
    finally:
        writer.close()
...
def record_episode(
    ...
    try:
        ...
        _write_video(frames, output_path, fps, frame_repeat=frame_repeat)
        print(
            f"\nVideo saved: {output_path} "
            f"({len(frames)} captured frames x {max(1, frame_repeat)} @ {fps}fps "
            f"= {len(frames) * max(1, frame_repeat) / fps:.1f}s)"
        )
        return RecordingResult(
            scenario_id=scenario_id,
            success=bool(result.success),
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Repeat each captured simulator frame N times in the MP4.",
    )
    parser.add_argument(
        "--require-success",
        action="store_true",
    ...
    parser.add_argument(
        "--hd",
        action="store_true",
        help="Shortcut for --camera robot0_frontview --resolution 1280 720; default fps 15.",
    )
    parser.add_argument(
        "--multi",
        action="store_true",
...
def apply_video_preset_args(args: argparse.Namespace) -> argparse.Namespace:
    if args.multi:
    ...
    args.frame_repeat = max(1, int(args.frame_repeat))
    return args
