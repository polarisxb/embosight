...
```bash
# 主视频：lemon，三视角拼接，展示识别、定位、执行全流程
python scripts/record_success_video.py \
  --output results/videos/embosight_kitchen_success_lemon.mp4 \
  --candidates fixed_lemon_001 \
  --multi \
  --fps 24 \
  --frame-repeat 2 \
  --min-frames 8

# 备用/补充视频：apple，正面高清，展示换物体后仍可运行
python scripts/record_success_video.py \
  --output results/videos/embosight_kitchen_success_apple.mp4 \
  --candidates fixed_apple_001 \
  --hd \
  --fps 24 \
  --frame-repeat 2 \
  --min-frames 8
...
`--fps 24` 控制 MP4 播放帧率；`--frame-repeat 2` 会把每一帧写入两次，让动作展示更平顺、视频时长更适合评审观看。如果觉得视频偏慢，把 `--frame-repeat` 改回 `1`。
